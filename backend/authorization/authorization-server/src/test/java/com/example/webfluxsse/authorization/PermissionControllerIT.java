...
class PermissionControllerIT {
  ...
  @Autowired private UserEventPermissionRepository permissionRepository;
  ...
  void setUp() {
    ...
    databaseClient
        .sql("ALTER SEQUENCE user_event_permissions_id_seq RESTART WITH 1")
        .fetch()
        .rowsUpdated()
        .block();
  }
  ...
  private Long createEvent(String title, String description) {
    ...
    databaseClient
        .sql(
            "INSERT INTO events (timestamp, title, description) VALUES (:timestamp, :title, :description)")
    ...
    return databaseClient
        .sql(
            "SELECT id FROM events WHERE timestamp = :timestamp AND title = :title AND description = :description ORDER BY id DESC LIMIT 1")
        .bind("timestamp", timestamp)
        .bind("title", title)
        .bind("description", description)
        .fetch()
  ...
  void shouldHandleDuplicatePermissionGrant() {
    ...
    webTestClient
        ...
        .exchange()
        .expectStatus()
        .is5xxServerError(); // Should fail due to unique constraint
  }
  ...
  void shouldGrantPermissionsForMultipleEvents() {
    ...
    PermissionController.GrantMultiplePermissionsRequest request =
        new PermissionController.GrantMultiplePermissionsRequest(
            java.util.List.of(event1Id, event2Id, event3Id), "user1");
