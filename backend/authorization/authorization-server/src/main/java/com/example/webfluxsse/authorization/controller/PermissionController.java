...
public class PermissionController {
  ...
  @Operation(
      ...
      description = "Returns all user-event permission mappings")
  @ApiResponses(
      value = {
        @ApiResponse(
            responseCode = "200",
  ...
  @Operation(
      ...
      description = "Returns all permissions granted to a specific user")
  @ApiResponses(
      value = {
        @ApiResponse(
            responseCode = "200",
  ...
  public Flux<UserEventPermission> getPermissionsForUser(
      @Parameter(description = "User identifier", example = "user123", required = true)
          @PathVariable
          String userId) {
    return permissionRepository.findByUserId(userId).map(PermissionMapper::toDto);
  }

  @Operation(
      summary = "Get permissions for an event",
      description = "Returns all users who have permission to access a specific event")
  @ApiResponses(
      value = {
        @ApiResponse(
            responseCode = "200",
            description = "Successfully retrieved event permissions",
            content =
                @Content(
                    mediaType = MediaType.APPLICATION_JSON_VALUE,
                    schema = @Schema(implementation = UserEventPermission.class)))
  ...
  public Flux<UserEventPermission> getPermissionsForEvent(
      @Parameter(description = "Event identifier", example = "42", required = true) @PathVariable
          Long eventId) {
    return permissionRepository.findByEventId(eventId).map(PermissionMapper::toDto);
  }

  @Operation(
      summary = "Get users for an event",
      description = "Returns list of user IDs who have permission to access a specific event")
  @ApiResponses(
      value = {
        @ApiResponse(
            responseCode = "200",
  ...
  @Operation(
      ...
      description = "Returns list of event IDs that a user has permission to access")
  @ApiResponses(
      value = {
        @ApiResponse(
            responseCode = "200",
            description = "Successfully retrieved event IDs",
            content =
                @Content(
                    mediaType = MediaType.APPLICATION_JSON_VALUE,
  ...
  @Operation(
      ...
      description = "Grants a user permission to access a specific event")
  @ApiResponses(
      value = {
        @ApiResponse(
            responseCode = "201",
  ...
  public Mono<UserEventPermission> grantPermission(
    ...
    UserEventPermissionEntity entity =
        new UserEventPermissionEntity(request.eventId(), request.userId());
    return permissionRepository
        .save(entity)
        .map(PermissionMapper::toDto)
        .doOnSuccess(
            savedPermission ->
                log.info("Successfully granted permission with id={}", savedPermission.id()))
        .doOnError(
            error ->
                log.error(
                    "Failed to grant permission for eventId={}, userId={}: {}",
                    request.eventId(),
                    request.userId(),
                    error.getMessage()));
  }
  ...
  @Operation(
      ...
      description = "Revokes a user's permission to access a specific event")
  @ApiResponses(
      value = {
  ...
  public Mono<Void> revokePermission(
    ...
    return permissionRepository
        .deleteByEventIdAndUserId(eventId, userId)
        .doOnSuccess(
            v ->
                log.info(
                    "Successfully revoked permission for eventId={}, userId={}", eventId, userId))
        .doOnError(
            error ->
                log.error(
                    "Failed to revoke permission for eventId={}, userId={}: {}",
                    eventId,
                    userId,
                    error.getMessage()));
  }
  ...
  @Operation(
      ...
      description = "Checks if a user has permission to access a specific event")
  @ApiResponses(
      value = {
        @ApiResponse(
            responseCode = "200",
  ...
  public Mono<PermissionCheckResponse> checkPermission(
      @Parameter(description = "Event identifier", example = "42", required = true) @RequestParam
          Long eventId,
      @Parameter(description = "User identifier", example = "user123", required = true)
          @RequestParam
          String userId) {
    ...
    return permissionRepository
        ...
        .doOnNext(
            response ->
                log.info(
                    "Permission check result: eventId={}, userId={}, hasPermission={}",
                    eventId,
                    userId,
  ...
  @Operation(
      ...
      description =
          "Checks which events from a list a user has permission to access. "
  ...
  public Mono<BatchPermissionCheckResponse> batchCheckPermissions(
      @RequestBody(
              ...
              required = true,
              content =
                  @Content(schema = @Schema(implementation = BatchPermissionCheckRequest.class)))
          @org.springframework.web.bind.annotation.RequestBody
          BatchPermissionCheckRequest request) {
    log.info(
        "Batch checking permissions: eventIds={}, userId={}", request.eventIds(), request.userId());

    return permissionRepository
        .findAllByUserIdAndEventIdIn(request.userId(), request.eventIds())
        ...
        .doOnNext(
            response ->
                log.info(
                    "Batch check result: userId={}, authorizedCount={}",
                    request.userId(),
                    response.authorizedEventIds().size()));
  }

  @Operation(
      summary = "Grant multiple permissions (Bulk)",
      description =
          "Grants a user permission to access multiple events in a single request. "
  ...
  @ApiResponses(
      value = {
        @ApiResponse(
            responseCode = "201",
            description =
                "Permissions granted successfully (returns only newly created permissions)",
            content =
                @Content(
                    mediaType = MediaType.APPLICATION_JSON_VALUE,
                    schema = @Schema(implementation = UserEventPermission.class))),
  ...
  public Flux<UserEventPermission> grantMultiplePermissions(
      @RequestBody(
              description = "Bulk permission grant request",
              required = true,
              content =
                  @Content(
                      schema = @Schema(implementation = GrantMultiplePermissionsRequest.class)))
          @org.springframework.web.bind.annotation.RequestBody
          GrantMultiplePermissionsRequest request) {
    log.info(
        "Granting permissions for multiple events: eventIds={}, userId={}",
        request.eventIds(),
        request.userId());
    ...
    return Flux.fromIterable(request.eventIds())
            ...
            entity ->
                permissionRepository
                    ...
                    .onErrorResume(
                        error -> {
                          log.warn(
                              "Permission already exists for eventId={}, userId={}, skipping",
                              entity.getEventId(),
                              entity.getUserId());
        ...
        .doOnNext(
            savedPermission ->
                log.info("Successfully granted permission with id={}", savedPermission.id()))
        .doOnComplete(
            () -> log.info("Completed bulk permission grant for userId={}", request.userId()));
  }

  public record GrantPermissionRequest(
      @Schema(description = "Event identifier", example = "42", required = true) Long eventId,
      @Schema(description = "User identifier", example = "user123", required = true)
          String userId) {}

  public record GrantMultiplePermissionsRequest(
      @Schema(
              description = "List of event identifiers",
              example = "[1, 2, 3, 4, 5]",
              required = true)
          java.util.List<Long> eventIds,
      @Schema(description = "User identifier", example = "user123", required = true)
          String userId) {
    public GrantMultiplePermissionsRequest {
      // Defensive copy: create immutable list to prevent external modification
      eventIds = eventIds == null ? java.util.List.of() : java.util.List.copyOf(eventIds);
    }
  }

  public record PermissionCheckResponse(
      @Schema(description = "Event identifier", example = "42") Long eventId,
      @Schema(description = "User identifier", example = "user123") String userId,
