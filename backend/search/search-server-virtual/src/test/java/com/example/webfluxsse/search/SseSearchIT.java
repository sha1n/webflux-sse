...
import com.example.webfluxsse.search.repository.jpa.EventRepository;
...
class SseSearchIT {
    ...
    void shouldSearchWithSseUsingPostRequestBody() throws Exception {
        // Create events
        EventEntity entity1 = eventRepository.save(EventMapper.toEntity(new Event(LocalDateTime.now(), "SSE Test Event", "Testing SSE streaming")));
        Event event1 = EventMapper.toDto(entity1);

        // Index in Elasticsearch
        elasticsearchRepository.save(entity1);
    ...
    void shouldFilterSseResultsBasedOnPermissions() throws Exception {
        // Create two events
        EventEntity entity1 = eventRepository.save(EventMapper.toEntity(new Event(LocalDateTime.now(), "User1 Event", "For user1")));
        EventEntity entity2 = eventRepository.save(EventMapper.toEntity(new Event(LocalDateTime.now(), "User2 Event", "For user2")));
        Event event1 = EventMapper.toDto(entity1);
        Event event2 = EventMapper.toDto(entity2);

        // Index in Elasticsearch
        elasticsearchRepository.save(entity1);
        elasticsearchRepository.save(entity2);
    ...
    void shouldSearchWithSseUsingGetQueryParams() throws Exception {
        // Create event
        EventEntity entity = eventRepository.save(EventMapper.toEntity(new Event(LocalDateTime.now(), "GET SSE Event", "Testing GET endpoint")));
    ...
    void shouldHandleEmptyQueryParameterInGetEndpoint() throws Exception {
        // Create event
        EventEntity entity = eventRepository.save(EventMapper.toEntity(new Event(LocalDateTime.now(), "Any Event", "Should be found")));
        Event event = EventMapper.toDto(entity);
    ...
    void shouldReturnEmptyStreamForUserWithNoPermissions() throws Exception {
        // Create event
        EventEntity entity = eventRepository.save(EventMapper.toEntity(new Event(LocalDateTime.now(), "Restricted Event", "No access")));
        elasticsearchRepository.save(entity);
