...
class SseSearchIT {
        ...
        eventRepository
            .save(
                EventMapper.toEntity(
                    new Event(LocalDateTime.now(), "SSE Test Event", "Testing SSE streaming")))
            .block();
  ...
  void shouldFilterSseResultsBasedOnPermissions() throws Exception {
    ...
    EventEntity entity1 =
        eventRepository
            .save(EventMapper.toEntity(new Event(LocalDateTime.now(), "User1 Event", "For user1")))
            .block();
    EventEntity entity2 =
        eventRepository
            .save(EventMapper.toEntity(new Event(LocalDateTime.now(), "User2 Event", "For user2")))
            .block();
    Event event1 = EventMapper.toDto(entity1);
  ...
  void shouldSearchWithSseUsingGetQueryParams() throws Exception {
    ...
    EventEntity entity =
        eventRepository
            .save(
                EventMapper.toEntity(
                    new Event(LocalDateTime.now(), "GET SSE Event", "Testing GET endpoint")))
            .block();
        ...
        post(urlEqualTo("/api/v1/permissions/batch-check"))
            ...
            .willReturn(
                aResponse()
                    ...
                    .withBody(
                        objectMapper.writeValueAsString(
                            Map.of("userId", "user1", "authorizedEventIds", Set.of(event.id()))))));
  ...
  void shouldHandleEmptyQueryParameterInGetEndpoint() throws Exception {
    ...
    EventEntity entity =
        eventRepository
            .save(
                EventMapper.toEntity(
                    new Event(LocalDateTime.now(), "Any Event", "Should be found")))
            .block();
    Event event = EventMapper.toDto(entity);
    ...
    stubFor(
        WireMock.get(urlEqualTo("/api/v1/permissions/user/user1/events"))
            .willReturn(
                aResponse()
                    .withStatus(200)
                    .withHeader("Content-Type", "application/json")
    ...
    Flux<Event> responseBody =
        webTestClient
            ...
            .uri(
                uriBuilder ->
                    uriBuilder.path("/api/v1/search/sse").queryParam("userId", "user1").build())
            .accept(MediaType.TEXT_EVENT_STREAM)
            .exchange()
            .expectStatus()
            .isOk()
  ...
  void shouldReturnEmptyStreamForUserWithNoPermissions() throws Exception {
    ...
    EventEntity entity =
        eventRepository
            .save(
                EventMapper.toEntity(
                    new Event(LocalDateTime.now(), "Restricted Event", "No access")))
            .block();
    elasticsearchRepository.save(entity).block();
