...
class EventControllerIT {
  ...
  @Test
  @DisplayName(
      "REST API should return all events from database in descending order by timestamp when multiple events exist")
  ...
  @Test
  @DisplayName(
      "REST API should return empty list with 200 OK status when no events exist in database")
  ...
  @Test
  @DisplayName(
      "SSE endpoint should stream events with proper content-type and JSON payload when events exist")
  void shouldStreamEventsViaSse() {
    ...
    StepVerifier.create(eventStream)
        .expectNextMatches(
            eventData ->
                eventData.contains("SSE Test Event") && eventData.contains("SSE Description"))
        .verifyComplete();
  }

  @Test
  @DisplayName(
      "SSE endpoint should continuously stream events every 2 seconds with real-time updates")
  void shouldContinuouslyStreamEvents() {
    Event event1 =
        new Event(LocalDateTime.now().minusMinutes(1), "Stream Event 1", "Stream Description 1");
  ...
  @Test
  @DisplayName(
      "SSE endpoint should detect and stream new events added to database during active connection")
  void shouldHandleEventUpdates() {
    ...
    eventRepository
        .save(EventMapper.toEntity(new Event(LocalDateTime.now(), "New Event", "New Description")))
  ...
  @Test
  @DisplayName(
      "REST API should return events sorted by timestamp DESC with newest events first regardless of insertion order")
  ...
  @Test
  @DisplayName(
      "REST API should return complete Event objects with all required fields (id, timestamp, title, description)")
  ...
  @Test
  @DisplayName(
      "POST API should create new event with 201 status and return complete event object with generated ID and timestamp")
  ...
  @Test
  @DisplayName(
      "POST API should persist event to database and make it available via GET API immediately after creation")
  ...
  @Test
  @DisplayName(
      "POST API should create events that become available via SSE streaming after creation")
