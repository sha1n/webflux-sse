...
public class SearchService {
  ...
  public Flux<Event> searchEventsForUser(String query, String userId) {
    ...
    return elasticsearchRepository
        ...
        .doOnComplete(
            () -> log.debug("Search completed for query='{}', userId='{}'", query, userId))
        .onErrorResume(
            error -> {
              log.error(
                  "Elasticsearch search failed for query='{}', userId='{}': {}",
                  query,
                  userId,
                  error.getMessage());
              return Flux.empty();
  ...
  private Mono<java.util.List<EventEntity>> checkPermissionsBatch(
      java.util.List<EventEntity> entities, String userId) {
    if (entities.isEmpty()) {
      return Mono.just(java.util.Collections.emptyList());
    }

    java.util.List<Long> eventIds =
        entities.stream().map(EventEntity::getId).collect(java.util.stream.Collectors.toList());

    log.debug("Checking permissions for {} events, userId='{}'", eventIds.size(), userId);

    // Call authorization-service via REST API
    return authorizationClient
        .checkBatchPermissions(eventIds, userId)
        ...
        .doOnNext(
            filteredEntities ->
                log.debug(
                    "Filtered to {} authorized events for userId='{}'",
