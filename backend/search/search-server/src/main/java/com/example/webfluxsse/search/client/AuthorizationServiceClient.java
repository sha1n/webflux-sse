...
public class AuthorizationServiceClient {
  ...
  public AuthorizationServiceClient(
    ...
    log.info(
        "AuthorizationServiceClient initialized with baseUrl: {}, timeout: {}", baseUrl, timeout);
  }

  public Mono<BatchPermissionCheckResponse> checkBatchPermissions(
      List<Long> eventIds, String userId) {
    ...
    return webClient
        ...
        .doOnSuccess(
            response ->
                log.debug(
                    "Batch permission check successful: userId={}, authorizedCount={}",
                    userId,
                    response.authorizedEventIds().size()))
        .onErrorResume(
            error -> {
              log.warn(
                  "Failed to check batch permissions for userId={}: {}. Returning empty permissions.",
                  userId,
                  error.getMessage());
  ...
  public Flux<Long> getEventIdsForUser(String userId) {
    ...
    return webClient
        ...
        .onErrorResume(
            error -> {
              log.warn(
                  "Failed to get event IDs for userId={}: {}. Returning empty list.",
                  userId,
                  error.getMessage());
              return Flux.empty();
            });
